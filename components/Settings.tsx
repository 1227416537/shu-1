...
const Settings: React.FC<SettingsProps> = ({ 
  ...
  const BACKUP_LAST_TIME_KEY = 'app_backup_last_time';
  const [lastBackupTime, setLastBackupTime] = useState<number>(() => {
    try { return Number(localStorage.getItem(BACKUP_LAST_TIME_KEY)) || 0; } catch { return 0; }
  });
  const daysSinceBackup = lastBackupTime > 0 ? Math.max(0, Math.floor((Date.now() - lastBackupTime) / 86_400_000)) : null;
  const lastBackupLabel = lastBackupTime > 0 ? new Date(lastBackupTime).toLocaleString('zh-CN') : '从未备份';
  const backupReminderDays = Math.max(1, Math.min(365, appSettings.backupReminderDays || 7));
  const backupNever = appSettings.backupReminderEnabled && lastBackupTime === 0;
  const backupOverdue = appSettings.backupReminderEnabled && daysSinceBackup !== null && daysSinceBackup >= backupReminderDays;
  ...
  const handleExportArchive = async () => {
    ...
    try {
      ...
      const fileName = `书伴-${date.getFullYear()}${pad(date.getMonth() + 1)}${pad(date.getDate())}-${pad(date.getHours())}${pad(date.getMinutes())}${pad(date.getSeconds())}.json`;
      // 记录备份时间，用于备份提醒
      try { localStorage.setItem(BACKUP_LAST_TIME_KEY, String(Date.now())); } catch { /* no-op */ }
    ...
    <div key="MAIN" className={`flex-1 flex flex-col p-6 pb-28 overflow-y-auto no-scrollbar relative ${containerClass} ${animationClass}`}>
      ...
      <section>
        ...
        <div className={`${cardClass} p-2 flex flex-col gap-2 rounded-2xl`}>
           ...
           {/* AI 伙伴称呼 */}
           <div className="p-3 rounded-xl flex items-center justify-between gap-3 cursor-default hover:bg-black/5 dark:hover:bg-white/5 transition-all">
              <div className="flex items-center gap-4 min-w-0">
                <div className={`${sectionIconClass} text-rose-400`}>
                  <Sparkles size={22} />
                </div>
                <div className="min-w-0">
                  <span className={`font-bold ${headingClass}`}>AI 伙伴称呼</span>
                  <div className="text-xs text-slate-500 mt-0.5">教程和默认聊天名里显示的称呼</div>
                </div>
              </div>
              <input
                value={appSettings.aiCompanionTerm ?? 'char'}
                onChange={(e) => updateSetting('aiCompanionTerm', e.target.value)}
                placeholder="char"
                maxLength={12}
                className={`w-24 text-right text-sm rounded-lg px-2 py-1.5 outline-none ${inputClass}`}
              />
           </div>
      ...
      <div className="mt-6 px-1">
        <div className={`${cardClass} p-3 rounded-2xl flex flex-col gap-3`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Bell size={18} className="text-slate-400" />
              <span className={`text-sm font-bold ${headingClass}`}>备份提醒</span>
            </div>
            {renderToggle(appSettings.backupReminderEnabled, () => updateSetting('backupReminderEnabled', !appSettings.backupReminderEnabled))}
          </div>
          <div className="flex items-center justify-between gap-2 text-xs text-slate-500">
            ...
            <input
              ...
              min={1}
              max={365}
              value={appSettings.backupReminderDays ?? 7}
              onChange={(e) => updateSetting('backupReminderDays', Math.max(1, Math.min(365, Number(e.target.value) || 7)))}
              className={`w-16 text-center text-xs rounded-lg outline-none ${inputClass}`}
            />
          </div>
          <div className="text-xs text-slate-500">上次备份：{lastBackupLabel}</div>
          {backupNever && (
            <div className="p-2 rounded-lg bg-amber-500/10 text-amber-600 dark:text-amber-400 text-xs">
              你还从未备份过，建议立即导出一份存档。
            </div>
          )}
          {backupOverdue && (
            <div className="p-2 rounded-lg bg-amber-500/10 text-amber-600 dark:text-amber-400 text-xs">
              距离上次备份已 {daysSinceBackup} 天，建议立即导出一份存档。
            </div>
          )}
        </div>
      </div>
