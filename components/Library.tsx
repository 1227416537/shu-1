...
import { getAiCompanionTerm } from '../utils/aiCompanionTerm';
...
输出: ^Chapter\\s*\\d+`;
  ...
  const charDisplayName = activeCharacter ? (activeCharacter.nickname || activeCharacter.name) : getAiCompanionTerm();
  ...
  const renderAvatar = (imageUrl: string | undefined, isDefaultUser: boolean, isDefaultChar: boolean, type: 'USER' | 'CHAR') => {
    ...
    if (type === 'USER') {
      ...
      if (isDefaultChar) return <ResolvedImage src={defaultCharImg} alt={`${getAiCompanionTerm()}默认形象`} className="w-full h-full object-cover" />;
                       ...
                       <div className="max-h-48 overflow-y-auto space-y-1">
                          ...
                          >
                             <div className="w-8 h-8 rounded-full overflow-hidden bg-slate-200 flex-shrink-0">
                                <ResolvedImage src={defaultCharImg} className="w-full h-full object-cover" alt={`${getAiCompanionTerm()}默认形象`} />
                             </div>
