...
import { getAiCompanionTerm, setAiCompanionTerm } from './utils/aiCompanionTerm';
...
const normalizeAppSettings = (raw: unknown): AppSettings => {
  ...
  const aiCompanionTerm =
    typeof source.aiCompanionTerm === 'string' && source.aiCompanionTerm.trim()
      ? source.aiCompanionTerm.trim()
      : DEFAULT_APP_SETTINGS.aiCompanionTerm;
  const backupReminderEnabled =
    typeof source.backupReminderEnabled === 'boolean'
      ? source.backupReminderEnabled
      : DEFAULT_APP_SETTINGS.backupReminderEnabled;
  const backupReminderDays =
    typeof source.backupReminderDays === 'number' && Number.isFinite(source.backupReminderDays) && source.backupReminderDays >= 1
      ? Math.min(365, Math.round(source.backupReminderDays))
...
const App: React.FC = () => {
  ...
  // 同步"AI 伙伴称呼"到模块缓存；称呼变化时（防抖 800ms）重建内置教程，保证教程里的术语一致
  const prevAiCompanionTermRef = useRef(appSettings.aiCompanionTerm);
  const tutorialRebuildTimerRef = useRef<number | null>(null);
  useEffect(() => {
    setAiCompanionTerm(appSettings.aiCompanionTerm);
    if (prevAiCompanionTermRef.current !== appSettings.aiCompanionTerm) {
      prevAiCompanionTermRef.current = appSettings.aiCompanionTerm;
      if (tutorialRebuildTimerRef.current !== null) window.clearTimeout(tutorialRebuildTimerRef.current);
      tutorialRebuildTimerRef.current = window.setTimeout(() => {
        const tutorial = createBuiltInTutorialBook();
        saveBookContent(tutorial.id, tutorial.fullText || '', tutorial.chapters || []);
        migrateTutorialImages(tutorial.chapters || []).then((migrated) => {
          saveBookContent(tutorial.id, tutorial.fullText || '', migrated);
        }).catch(() => { /* 图片迁移失败则保留 data-URL fallback */ });
        setBooks((prev) => prev.map((b) =>
        b.id === tutorial.id
          ? { ...compactBookForState(tutorial), progress: b.progress, lastRead: b.lastRead }
          : b
      ));
      }, 800);
    }
    ...
    const runProactiveCheck = async (plannedAt: number) => {
      ...
      try {
        ...
        const result = await runConversationGeneration({
          ...
          characterRealName: activeCharacter.name?.trim() || getAiCompanionTerm(),
          characterNickname: activeCharacter.nickname?.trim() || activeCharacter.name?.trim() || getAiCompanionTerm(),
