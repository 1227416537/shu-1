...
import { applyAiCompanionTermToText } from './aiCompanionTerm';
import {
  IMG_P1_0, IMG_P1_1,
...
export const BUILT_IN_TUTORIAL_VERSION = 4.8;
...
1.适配本地部署
构建产物改为相对路径，放到任意目录或子路径都能直接运行；新增离线缓存，安装为 PWA 后断网也能阅读。

2.更名
应用更名为"书伴"。

3.教程新增"第七章 多设备与数据备份（无需服务器）"章节，手机和电脑之间的数据迁移方法详见该章。

4.新增功能
AI 伙伴称呼可自定义（设置 → AI 伙伴称呼）；新增备份提醒（设置 → 备份提醒）。
...
这是一个和 char 一起阅读的小工具，本意是出于个人原因想提升阅读量做的，名字叫"书伴"——希望它能成为你读书时身边的一个伙伴。由于重点是阅读所以把聊天功能做得极简轻量，群聊和表情包功能是不支持的。注意勤备份，所有数据（书籍、聊天记录、笔记、设置等）都存储在浏览器本地。如果清除浏览器数据或更换设备，这些内容会丢失。建议定期在设置的"存储分析"中导出一份存档文件，以防万一。手机和电脑之间互传这份存档文件即可完成数据迁移，不需要服务器。
...
const CH1_BLOCKS: ReaderContentBlock[] = [
  text('第一章 快速开始'),
  text(`这是一个和 char 一起阅读的小工具，本意是出于个人原因想提升阅读量做的，名字叫"书伴"——希望它能成为你读书时身边的一个伙伴。由于重点是阅读所以把聊天功能做得极简轻量，群聊和表情包功能是不支持的。注意勤备份，所有数据（书籍、聊天记录、笔记、设置等）都存储在浏览器本地。如果清除浏览器数据或更换设备，这些内容会丢失。建议定期在设置的"存储分析"中导出一份存档文件，以防万一。手机和电脑之间互传这份存档文件即可完成数据迁移，不需要服务器。
...
/*  第七章 多设备与数据备份（无需服务器）                                    */
/* ------------------------------------------------------------------ */
const CH7_CONTENT = `第七章 多设备与数据备份（无需服务器）

书伴的所有数据（书籍、阅读进度、聊天记录、笔记、人设、设置）都保存在浏览器本地，不经过任何服务器。不同设备（手机 / 电脑）之间的数据互不相通，需要用"存档文件"在设备之间迁移。

一、把电脑上的数据同步到手机

1. 电脑端：打开「设置」，点页面底部的「导出存档文件」，得到"书伴-日期.json"文件。
2. 把文件传到手机（微信文件传输助手 / QQ / 邮箱 / 网盘都可以）。
3. 手机端：打开书伴 →「设置」→ 点「导入存档文件」→ 选择刚才的文件。
4. 导入会覆盖当前设备上已有的所有数据（会先弹窗确认），完成后自动刷新。

二、把手机上的数据同步到电脑

1. 手机端：「设置」→「导出存档文件」，保存到手机。
2. 把文件传到电脑（微信 / 数据线 / 网盘）。
3. 电脑端：「设置」→「导入存档文件」→ 选择该文件。

三、备份建议

1. 数据只在浏览器本地，清理浏览器数据、换浏览器或重装系统都会丢失。建议每隔一段时间导出一份存档。
2. 「设置」→「存储分析」可以查看各类数据占了多少空间。
3. 想在同一个 WiFi 下直接用手机访问电脑上的书伴，见项目里的"本地部署指南.md"。`;

const CH7_BLOCKS: ReaderContentBlock[] = [
  text('第七章 多设备与数据备份（无需服务器）'),
  text(`第七章 多设备与数据备份（无需服务器）
...
const CH8_CONTENT = `第八章 免费TTS API 部署指南
...
const CH8_BLOCKS: ReaderContentBlock[] = [
  text('免费TTS API 部署指南'),
...
const TUTORIAL_CHAPTERS: Chapter[] = [
  ...
  { title: '第七章 多设备与数据备份', content: CH7_CONTENT, blocks: CH7_BLOCKS },
  { title: '第八章 免费TTS部署教程', content: CH8_CONTENT, blocks: CH8_BLOCKS },
];
...
export function createBuiltInTutorialBook(): Book {
  // 根据"AI 伙伴称呼"设置，把教程里的 char 术语替换成用户自定义称呼
  const chapters = TUTORIAL_CHAPTERS.map((ch) => ({
    ...ch,
    content: applyAiCompanionTermToText(ch.content),
    blocks: ch.blocks.map((b) =>
      b.type === 'text' ? { ...b, text: applyAiCompanionTermToText(b.text) } : b
    ),
  }));
  const fullText = computeFullText(chapters);
  return {
