...
import { getAiCompanionTerm } from './aiCompanionTerm';
...
const normalizeQuotePayload = (value: unknown, fallbackTimestamp: number): ChatQuotePayload | undefined => {
  ...
  return {
    ...
    senderName: compactText(source.senderName) || (source.sender === 'user' ? DEFAULT_USER_NAME : getAiCompanionTerm()),
    content,
    timestamp,
  };
};
...
const normalizeChatBubble = (value: unknown): ChatBubble | null => {
  ...
  const fallbackPromptRecord =
    source.sender === 'user'
      ...
      : buildCharacterPromptRecord(getAiCompanionTerm(), content, timestamp);
  return {
